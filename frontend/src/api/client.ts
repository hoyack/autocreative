...
// Per 21-RESEARCH.md Pattern 2 + Pitfall #6: baseUrl stays empty by default so
// the Vite dev proxy (vite.config.ts forwards /api/* to http://localhost:8000)
// and eventual same-origin production mount both work without client-side URL
// rewriting. Set VITE_API_URL only when pointing at a non-default backend.
//
