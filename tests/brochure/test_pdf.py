...
def test_pdf_page_size_tracks_png_at_300dpi() -> None:
    """Mediabox = (PNG_W * 72/300, PNG_H * 72/300) — 1:1 with the source PNG.

    Quick task 260425-nwj v2: the PDF page size now follows the PNG's actual
    dimensions, so a 1080×1920 portrait panel produces a 3.6×6.4 in PDF and
    a 3376×2626 bleed canvas produces an 11.25×8.75 in PDF. Image is never
    stretched or letterboxed — it fills the page 1:1.

    Earlier tries (forced 11.25×8.75 bleed; forced 11×8.5 letter trim) both
    distorted the image when the renderer's PNG aspect didn't match the
    chosen page size.
    """
    pdf_bytes = assemble_brochure_pdf(_fake_sheet_png(), _fake_sheet_png())
    reader = PdfReader(io.BytesIO(pdf_bytes))
    pt_per_px = 72.0 / 300.0
    expected_w_pt = BLEED_CANVAS_WIDTH * pt_per_px   # ~810
    expected_h_pt = BLEED_CANVAS_HEIGHT * pt_per_px  # ~630
